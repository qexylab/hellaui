import React, {createRef, FC, PropsWithChildren, useMemo, useRef, useState} from 'react';
...
export const Tab: FC<PropsWithChildren<ITab>> = ({
  ...
  const [visibilityMap, setVisibilityMap] = useState<{ [index: string]: boolean }>(
  ...
  const tabsWithRef: TabWithRefProps[] = useMemo(() => {
    return tabs.map((tab: TabProps) => ({ ...tab, ref: createRef<HTMLButtonElement>() }));
  }, [tabs]);
  ...
  return (
      <div
          ...
          style={{
            ...
            alignItems: "center",
            width: '100%',
            boxShadow: `inset 0 -2px 0 0 ${underline ? theme_color.gray : 'transparent'}`,
            overflow: "hidden",
            height: height,
            maxHeight: height,
      ...
      >
        ...
        <div ref={underlineRef} aria-hidden style={{
          position: "absolute",
          bottom: 0,
          display: "flex",
